...
#Creates a PCA Data object that inherits fields from Data class
class PCAData(Data):
	def __init__(self,headers,pdata,evals,evecs,means):
		...
		#Populate PCAData object with fields from Data class
		...
		print "Head2Raw",self.header2raw
		print "Header2Matrix",self.header2matrix
		print "Raw headers", self.raw_headers

	#Returns copy of eigenvalues as single np matrix
	def get_eigenvalues(self):
		evalcopy=deepcopy(self.evals)
		return evalcopy

	#Returns copy of eigenvectors as single np matrix
	def get_eigenvectors(self):
		eveccopy = deepcopy(self.evecs)
		return eveccopy

	#Returns mean for each column as single np matrix
	def get_data_means(self):
		data_mean_copy=deepcopy(self.data_means)
		return data_mean_copy

	#Returns copy of headers from original data used to generate predicted data
